...
namespace Naive.Serializer.UnitTests
{
    public class NaiveSerializerPerformanceTests
    {
        ...
        static object[] TestPerformanceCases =
        {
            ...
            new []{ 10000, "list[10] string(1)", (object)new List<string> { "*", null, "*", "*", "*", "*", "*", "*", "*", "*" } },
            new []{ 10000, "list[10] byte[10]", (object)Enumerable.Range(0, 10).Select(x => Enumerable.Range(0, 10).Select(x => (byte)x).ToArray()).ToList() },
